# 应用架构与修改边界

这个项目仍使用原生 HTML、CSS 和 JavaScript。当前目标不是引入框架，而是让核心业务规则保持单一来源，让页面层和数据层不会各自实现一套学习逻辑。

## 运行时加载顺序

`index.html` 必须按以下顺序加载脚本：

1. `scheduler.js`
2. `progress-model.js`
3. `daily-plan.js`
4. `taxonomy.js`
5. `romanian-text.js`
6. `quiz-engine.js`
7. `api.js`
8. `telemetry.js`
9. `auth.js`
10. `app.js`
11. `pwa.js`

依赖顺序由 `tools/verify_architecture.js` 自动检查。

## 模块职责

### `scheduler.js`

负责单张卡片的记忆调度规则：卡片状态、复习间隔、记忆强度、需加强状态、复习阶段字段兼容，以及防止进度倒退的判断。

这里不能访问 DOM、Supabase 或每日队列状态。

### `progress-model.js`

负责学习进度模型：合并两份进度、选择较可靠的调度快照、合并时间证据、计算存储等级和读取语法练习计数。

页面内存与云端数据必须调用这个模块，不能在 `app.js` 或 `api.js` 中重新实现合并规则。

### `daily-plan.js`

负责每日队列的纯规划算法：去重、阶段排序、分层选卡和固定配额开放队列组合。

它不判断某个词是否到期或是否需加强。主题筛选判断留在 `app.js`，规划器只处理调用方提供的筛选结果。

### `taxonomy.js`

负责词条的结构化内容模型：主题、词性、词汇单位、语法对象、精确 CEFR、语域和核对状态。它同时负责兼容旧 `cat`/`hint` 数据、格式化学习者可见的语法信息，以及识别模板行、未分类内容、单复数矛盾和缺失双语例句。这里是纯模块，不访问 DOM、用户状态或云端。

### `romanian-text.js`

负责罗语重音推测、重音安全渲染和语法提示回退。它是纯模块，不访问 DOM、用户状态或云端。

### `quiz-engine.js`

负责测验的纯规则：可信题库筛选、相对难度分层、综合诊断蓝图、同类干扰项、罗语听写判分、真人录音元数据校验和分维度结果汇总。它不能访问 DOM、播放音频或写入学习进度；`app.js` 只负责把这些规则接入交互流程。

真人录音必须同时提供 `audio_url`、`audio_kind: "human"`、`audio_source` 和 `audio_license`。生产环境只接受 HTTPS；本机预览可使用 localhost HTTP。缺少来源或许可时只能回退到设备合成音，界面不得标成真人录音。

### `api.js`

负责 Supabase、本机存储、离线回退和待同步队列。它可以把数据库行转换成进度对象，但不能拥有调度或进度合并规则。

每日状态和单词进度都必须先写入按事件 ID 独立存储的本机 outbox，再调用幂等 RPC。
每日计数使用 base→target 增量，队列使用集合增删增量；禁止读取后直接 upsert 整份
`daily_queue`/`daily_log` 快照，也禁止用客户端 `updated_at` 决定覆盖顺序。

### `auth.js`

负责登录、注册、离线登录和退出。

### `telemetry.js`

负责客户端异常去重、隐私清洗和限流，再通过 `api.js` 写入故障汇总。不得上传邮箱、单词内容或完整 URL。

### `app.js`

负责页面状态、DOM 渲染、用户交互和跨模块编排。新增功能应优先调用纯模块；只有确实依赖页面状态或 DOM 的流程才放在这里。

### `pwa.js`

负责 Service Worker 注册、版本检测和用户确认刷新。`sw.js` 只负责缓存策略与激活消息。

## 不变量

- 每日队列固定为用户目标配额。
- 正式到期复习及其重新学习步骤优先于初次学习步骤；两者都优先于新词。
- 有正式复习时，初次学习步骤不得占用每日复习通过目标；正式复习清空后，学习/新词的准确作答才可补足当日目标。
- “不认识”和“模糊”不会完成今日任务；只有“认识”完成队列词。
- 每日通过目标与每日新词上限是两个独立约束；默认分别为 200 和 30。
- 回看历史卡片只读；撤销必须精确恢复进度、每日状态，并以更高版本的待同步修正覆盖旧写入。
- 已有成熟进度不能被较新的空白或低阶段记录覆盖。
- 调度器、进度模型和每日规划器不得访问 DOM 或直接写云端。
- 主题、词性和词汇单位必须独立存储；旧 `cat` 只能作为兼容来源，不能再同时表达多个维度。
- 宽泛等级（例如 `A1-B2`）不能伪装成精确 CEFR；没有核实的等级保持为空，并在界面中隐藏。
- 综合诊断只使用已核对或已修订词条；词库难度是题库内相对分层，结果不得表述为 CEFR 定级或完整听力水平。
- 空的可选元数据不显示占位噪声；尚未学习的词只显示“尚未开始学习”，不显示“阶段 0”或“未安排”。
- 正式词库不能包含模板表头、中文罗语字段、规范化重复、待归类主题或待核对词性。
- 修改运行时模块后必须同步更新脚本版本和 Service Worker 缓存版本。

## 验证

在仓库根目录运行：

```bash
node tools/run_checks.js
npm --prefix "app build" run build:web
npm --prefix "app build" run test:e2e
```

GitHub Actions 会在每个 PR 和 `main` 推送时执行同样的检查。

`tools/verify_dead_code.js` 还会检查只声明但没有运行时调用点的函数。确实需要作为浏览器控制台或兼容 API 保留的入口，必须在该检查中写明保留原因。
