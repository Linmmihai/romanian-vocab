...
- **每日任务队列**：默认每天处理 200 个词，依次安排到点学习步骤、正式到期复习，再用剩余空位学习未学词；当天固定队列不会因为延迟重试而扩充到第 201 个。
...
每日任务队列按 Anki 式阶段出卡：已到点的学习/重新学习步骤优先，其次是正式到期复习，最后才是真正未学过的新词。只要前两个阶段仍有到期卡片，新词就不会穿插出现。默认每日通过目标为 200，同时另设每日新词上限 30；新词上限只控制第一次引入的卡片，不会阻止已经开始学习的卡片按时重试。新词答错只会留在“学习中”并按短间隔重试；已经毕业进入复习阶段的词再次答错，会进入重新学习并提高后续出题优先级。
...
- 出卡顺序：已到点的学习步骤 → 正式到期复习 → 新词；任何前序队列未清空时都不能提前显示后序卡。
