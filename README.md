...
- **学习统计**：显示每日学习量、连续学习天数、正确率、分类掌握率和高频错词。
...
每日任务队列按 Anki 式阶段出卡：已到点的学习/重新学习步骤优先，其次是正式到期复习，最后才是真正未学过的新词。新词答错只会留在“学习中”并按短间隔重试；已经毕业进入复习阶段的词再次答错，会进入重新学习并提高后续出题优先级。一个词一旦产生卡片答题记录，就不会再显示为“今日待学”。
