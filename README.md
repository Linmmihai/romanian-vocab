...
这是一个面向中文使用者的罗马尼亚语词汇学习应用。应用结合了卡片记忆、间隔复习、测验、错题本、学习统计和 Android APK 打包功能。

界面目前主要使用中文，因为目标用户是从中文学习罗马尼亚语。

## 主要功能

- **每日新词队列**：根据每日目标自动生成当天需要学习的未学词。
- **卡片记忆**：点击或滑动卡片查看罗马尼亚语、重音标记和语法信息。
- **间隔复习**：学过的词会按复习阶段安排下一次复习，例如 20 分钟、1 天、2 天、4 天等。
- **记忆追踪**：记录词条是否看过、是否认识、答题次数、复习阶段、下次复习时间、错题次数、连续错误和最近错误时间。
- **测验模式**：支持中罗互译，以及名词复数、动词类型、重音等专项练习。
- **错题本**：答错的词会进入错题本，连续答对 3 次后可以移出。
...
每日新词队列只应该选择真正未学过的词。一个词一旦被标记为认识或被答题记录覆盖，就会离开新词池，之后由间隔复习系统安排。
